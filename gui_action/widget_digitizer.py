...
class DigitizerWidget(QtWidgets.QWidget):
    ...
    def connect_signals_slots(self):
        ...
        logging.info('connecting all signals digitizerwidget')
    ...
    def disconnect_signals_slots(self):
        ...
        logging.info('disconnecting all signals digitizerwidget')
    ...
    def set_measurement_mode(self, mode):
        ...
        logging.info(f'changing digitizer measuring mode to {mode}')
    ...
    def set_data_channel(self, channel):
        ...
        logging.info(f'digitizerwidget: setting data channel to {channel}')
    ...
    def set_jitter_channel(self, channel):
        logging.info(f'setting digitizer jitter correction channel to {channel}')
    ...
    def jitter_correction_alignment(self):
        ...
        if not self.ui.checkBox_jitter_correction_alignment.isChecked:
            logging.info('enabling jitter correction for digitizer')
            ...
            if self.ui.comboBox_time_range_alignment.currentText() == '40 ms':
                ...
                logging.info('cannot measure longest time with multiple channels, reverting to shortest time range')
            index = self.ui.comboBox_time_range_alignment.findText('40 ms')
            if index != -1:
                logging.info('removing longest timerange from digitizer timerange alignment')
            ...
            logging.info('disabling jitter correction for digitizer')
            ...
            if self.digitizer.model == 'DT5724F':
                if self.ui.comboBox_time_range_alignment.findText('40 ms') == -1:
                    logging.info('adding longest timerange to digitizer')
    ...
    def jitter_correction_experiment(self):
        ...
        if not self.ui.checkBox_jitter_correction_experment.isChecked:
            if self.ui.comboBox_time_range_experiment.currentText() == '40 ms':
                ...
                logging.info('digitizerwidget: set chosen timerange to shortest')
            index = self.ui.comboBox_time_range_alignment.findText('40 ms')
            if index != -1:
                logging.info('digitizerwidget: removing longest timerange from timerange experiment')
                self.ui.comboBox_time_range_alignment.removeItem(index)
        else:
            if self.digitizer.model == 'DT5724F':
                if self.ui.comboBox_time_range_experiment.findText('40 ms') == -1:
                    logging.info('digitizerwidget: adding longest timerange to experiment')
                    self.ui.comboBox_time_range_alignment.addItem('40 ms')

    @pyqtSlot(str)
    def set_time_range(self, timerange):
        """ Parse the selected timerange to the digitizer """
        logging.info(f'setting digitizer time range to {timerange}')
        value = TIMERANGES[self.digitzer.model].index(timerange)
    ...
    def set_single_photon_compression(self, factor):
        ...
        logging.info(f'digitizerwidget: selected compressionfactor {factor}')
        try:
            compressionfactor = COMPRESSIONFACTORS[self.digitizer.model][factor]
        except KeyError:
            logging.error(f'digitizerwidget: compressionfactor {factor} not found in compressionfactors dict, setting'
                          f'to no compression')
            compressionfactor = 1
