...
class Digitizer(DigitizerHandle):
    ...
    def record_length(self):
        ...
        logging.info(f'record length = {return_value.value} samples per channel')
    ...
    def record_length(self, value):
        ...
        if value < 0:
            value = 0
            logging.warning(f'specified record length too small. set to minimum value: {value}')
        elif value > 10:
            value = 10
            logging.warning(f'specified record length too big, set to maximum value: {value}')
        self.rl = value
        ...
        logging.info(f'set record length to {record_length} samples per channel')
    ...
    def manual_record_length(self, value):
        ...
        handle_error(_lib.CAEN_DGTZ_SetRecordLength(self._handle, set_value))
        logging.info(f'set record length to manual value of {value} samples')
    ...
    def max_num_events_blt(self):
        ...
        logging.info(f'Max num events per block transfer = {return_value.value}')
        return return_value.value
    ...
    def buffer_organization(self):
        ...
        logging.info(f'Number of buffers per channel memory = {1 << value.value}')
    ...
    def buffer_organization(self, set_value):
        ...
        logging.info(f'Number of buffers per channel memory set to {1 << set_value.value}')
    ...
    def free_event(self):
        ...
        logging.info('CAEN DT57xx frees event')
    ...
    def free_readout_buffer(self):
        ...
        logging.debug('CAEN DT57xx frees memory allocated by the MallocReadoutBuffer function')
    ...
    def send_sw_trigger(self):
        ...
        """
        logging.info('CAEN DT57xx software trigger sent')
    ...
    def finish_measurement(self):
        ...
        logging.debug("finishing measurement")
    ...
    def allocate_event(self):
        ...
        """
        logging.info('CAEN DT57xx allocates event')
    ...
    def get_num_events(self):
        ...
        """
        logging.debug('CAEN DT57xx scans the readout buffer and gets the number of events')
    ...
    def decode_event(self, evtptr):
        ...
        logging.debug(f'Decode Event {evtptr}')
        ...
        for channel in self.active_channels:
            ...
            break
        data = np.zeros((len(self.active_channels), channel_size))

        for channel in self.active_channels:
            data[channel] = self.event.contents.DataChannel[channel][0:channel_size]
        return data
    ...
    def read_data(self, readoutmode=definitions.ReadMode.POLLING_MBLT.value):
        ...
        """
        logging.debug('CAEN DT57xx Read data')
        self.buffer_size = c_uint32(0)
        ...
        logging.debug(f"ReadData self.buffersizesize.value : {self.buffer_size}")
    ...
    def read_readout_status(self):
        ...
        """
        logging.info('Read CAEN DT57xx Acquisition Status Register')
        address = 0xEF04
        status = c_int32(0)
    ...
    def malloc_readout_buffer(self):
        ...
        """
        logging.debug('CAEN DT57xx allocates memory buffer for the data block transfer')
        size = c_uint32(0)
...

        ...
        """
        logging.info('CAEN DT57xx clearing all data in the digitizer')
    ...
    def events_stored(self):
        ...
        """
        logging.info('Read CAEN DT57xx Events Stored Register')
        address = 0x812C
        events = c_int32(0)
    ...
    def sw_start_acquisition(self):
        ...
        """
        logging.debug('DT57XX Started acquisition with software command')
    ...
    def sw_stop_acquisition(self):
        ...
        logging.debug('DT57XX Stopped acquisition with software command')
